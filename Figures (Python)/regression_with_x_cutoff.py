...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 31 01:18:41 2024
...
# Filter the data based on the x cutoff
mask = x.squeeze() > x_cutoff
x_filtered = x[mask]
y_filtered = y[mask]
...
axes[1].axvline(x=x_cutoff, color='orange', linestyle='--', linewidth=2, label=f'x cutoff = {x_cutoff}')
...
plt.tight_layout()
plt.savefig("regression_with_x_cutoff.pdf", format='pdf')
plt.show()
