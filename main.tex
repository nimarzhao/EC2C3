\documentclass[oneside,10pt]{book}
\usepackage{graphicx} % Required for inserting images
\usepackage[T1]{fontenc} % Nice font rendering
\usepackage[a4paper, portrait, margin=1.5in]{geometry}

% Math
\usepackage{amsmath,amssymb,amsthm,derivative}
\usepackage{txfonts,times}
\input{math_commands/probability_functions}
\input{math_commands/analysis functions}
\input{math_commands/theorems}

\usepackage{varioref}
\usepackage{hyperref}
\usepackage{cleveref}   % in this order please
\usepackage{tikz} % Diagrams
\usepackage{enumerate} % numbered bullet points
\usepackage[shortlabels]{enumitem}
\usepackage[dvipsnames]{xcolor}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{array}

% CODE
\usepackage{listings}
\usepackage[most]{tcolorbox}
\usepackage{inconsolata}

\newtcblisting[auto counter]{sexylisting}[2][]{sharp corners,
    fonttitle=\bfseries, colframe=gray, listing only, 
    listing options={basicstyle=\fontsize{9}{9}\ttfamily,language=java}, 
    title=Listing \thetcbcounter: #2, #1}


\title{EC2C3 Econometrics I}
\date{July 2024}
\author{Nimar C. Zhao\footnote{Department of Economics, London School of Economics and Political Science}}

\begin{document}
\maketitle
\tableofcontents
\newpage

\input{chapters/selection}
\input{chapters/regression}
\input{chapters/inference}
\input{chapters/measurement}
\input{chapters/research_design}
\input{chapters/endogeneity}
\input{chapters/panel_data}
\input{chapters/further_methods}

\end{document}

